<template>
  <v-container fluid class="pa-6 fill-height bg-slate-50 overflow-y-auto align-start">
    <v-row justify="center" class="w-100 ma-0">
      <v-col cols="12" xl="11">
        <!-- HEADER BANNER -->
        <v-card elevation="12" class="rounded-xl border-top-primary overflow-hidden mb-6">
          <v-toolbar color="surface" flat class="px-6 py-4 border-bottom d-flex align-center flex-wrap ga-4" height="auto">
            <div>
              <div class="d-flex align-center">
                <v-icon color="primary" class="mr-2" size="28">mdi-history</v-icon>
                <h1 class="text-h5 font-weight-bold text-slate-800">
                  HISTORIAL Y RELACIONES DEL MPP
                </h1>
              </div>
              <div class="text-caption text-slate-500 mt-1">
                Explore y compare las versiones de procedimientos, sus marcos legales, indicadores y flujos operacionales mapeados en el sistema.
              </div>
            </div>
            <v-spacer></v-spacer>
            <div class="d-flex align-center flex-wrap ga-3 flex-grow-1 flex-md-grow-0 justify-md-end">
              <!-- Filtro de estado -->
              <v-select
        ...
        <!-- CARGANDO / LOADING -->
        <v-card v-if="loadingGlobal" class="rounded-xl pa-8 text-center" elevation="4">
          <v-progress-circular indeterminate color="primary" size="64" class="mb-4"></v-progress-circular>
          <div class="text-subtitle-1 text-grey-darken-1">Cargando base de conocimiento e historial de versiones...</div>
        </v-card>

        <!-- SIN DATOS / NO DATA -->
        <v-card v-else-if="filteredProcesos.length === 0" class="rounded-xl pa-8 text-center" elevation="4">
          <v-icon size="64" color="grey" class="mb-4">mdi-database-off</v-icon>
          <div class="text-subtitle-1 text-grey-darken-1 font-weight-bold">No se encontraron procesos ni procedimientos relacionados</div>
          <div class="text-caption text-grey">Pruebe ajustando los filtros de búsqueda o de estado.</div>
        </v-card>
          ...
          >
            ...
            <v-expansion-panel-title class="py-4">
              <v-row align="center" no-gutters>
                <v-col cols="12" md="8" class="d-flex align-center flex-wrap ga-2">
                  <v-chip size="small" color="primary" variant="flat" class="font-weight-black uppercase px-3">
                    Proceso: {{ proceso.codigo || `#${proceso.id_proceso}` }}
                  </v-chip>
                  <span class="text-h6 font-weight-bold text-slate-800">{{ proceso.nombre }}</span>
                </v-col>
                <v-col cols="12" md="4" class="text-md-right mt-2 mt-md-0 d-flex justify-md-end flex-wrap ga-2">
                  <v-chip size="x-small" color="info" variant="tonal" class="font-weight-bold uppercase" prepend-icon="mdi-account-tie">
                    Resp: {{ getProcessResponsible(proceso.id_proceso) }}
                  </v-chip>
                  <v-chip size="x-small" color="secondary" variant="tonal" class="font-weight-bold" prepend-icon="mdi-file-tree">
                    {{ getProcedimientosCount(proceso.id_proceso) }} Procedimientos
                  </v-chip>
                </v-col>
              </v-row>
            ...
            <v-expansion-panel-text class="bg-slate-50 border-top pa-2">
              ...
              <div class="mb-4 pa-4 bg-white rounded-lg border">
                <div class="text-subtitle-2 font-weight-black text-primary mb-1">Descripción del Proceso:</div>
                <p class="text-body-2 text-slate-700 mb-3">{{ proceso.descripcion || 'Sin descripción detallada.' }}</p>
                
                <div class="text-subtitle-2 font-weight-black text-primary mb-1">Unidades Participantes del Proceso:</div>
                <div class="d-flex flex-wrap ga-2">
                  ...
                  <span v-if="!proceso.unidades || proceso.unidades.length === 0" class="text-caption text-grey italic">No hay unidades específicas vinculadas.</span>
                </div>
              </div>

              <!-- TABLA DE PROCEDIMIENTOS AGRUPADOS POR NOMBRE/CÓDIGO BASE (VERSIONADO) -->
              <div class="text-subtitle-1 font-weight-black text-slate-700 px-2 mb-3">Historial de Procedimientos:</div>
              
              <v-table class="rounded-lg border bg-surface overflow-hidden mb-3">
                <thead class="bg-indigo-lighten-5">
                  <tr class="text-uppercase text-caption font-weight-bold">
                    <th class="font-weight-bold text-left" width="140">Código</th>
                    <th class="font-weight-bold text-left">Nombre del Procedimiento</th>
                    <th class="font-weight-bold text-center" width="180">Versiones Disponibles</th>
                    <th class="font-weight-bold text-center" width="140">Última Versión</th>
                    <th class="font-weight-bold text-center" width="130">Estado Actual</th>
                    <th class="font-weight-bold text-center" width="140">Detalles</th>
                  </tr>
                </thead>
                <tbody>
                  <tr v-if="getProcedimientosGrouped(proceso.id_proceso).length === 0">
                    <td colspan="6" class="text-center py-6 text-grey text-caption">
                      No se encontraron procedimientos en este proceso con los filtros aplicados.
                    </td>
                  </tr>
                  <tr
                    v-for="grupo in getProcedimientosGrouped(proceso.id_proceso)"
                  ...
                  >
                    ...
                    <td class="text-center">
                      <v-chip size="x-small" color="primary" variant="flat" class="font-weight-bold">
                        v{{ getProcedureVersionString(grupo.selectedVersionId, grupo.versiones) }}
                      </v-chip>
                    </td>
                    <td class="text-center">
                      <v-chip
                        size="x-small"
                        :color="getProcedureStatusColor(grupo.selectedVersionId, grupo.versiones)"
                        variant="tonal"
                        class="font-weight-bold text-uppercase"
                      >
                        {{ getProcedureStatus(grupo.selectedVersionId, grupo.versiones) }}
                      </v-chip>
                    </td>
                    <td class="text-center">
                      <v-btn
    ...
    >
      <div v-if="selectedProcedure" class="d-flex flex-column fill-height bg-white">
        <!-- CABECERA DEL DRAWER -->
        <v-toolbar color="surface" flat class="px-6 py-4 border-bottom flex-shrink-0" height="auto">
          <div class="flex-grow-1 mr-4">
            <div class="d-flex align-center flex-wrap ga-2">
              <v-chip size="x-small" color="secondary" variant="flat" class="font-weight-black uppercase">
                {{ selectedProcedure.codigo || `PROC-${selectedProcedure.id_procedimiento}` }}
              </v-chip>
              <v-chip size="x-small" color="primary" variant="tonal" class="font-weight-black">
                Versión {{ selectedProcedure.version || '1.0' }}
              </v-chip>
              <v-chip size="x-small" :color="selectedProcedure.estado === 'Activo' ? 'success' : 'grey'" variant="tonal" class="font-weight-black uppercase">
                {{ selectedProcedure.estado || 'Activo' }}
              </v-chip>
            </div>
            <h2 class="text-h6 font-weight-bold text-slate-800 mt-2 line-clamp-2">
              {{ selectedProcedure.nombre }}
            </h2>
          </div>
          <v-btn icon="mdi-close" variant="tonal" size="small" @click="drawerOpen = false"></v-btn>
        </v-toolbar>

        <!-- PESTAÑAS DENTRO DEL DRAWER -->
        <v-tabs v-model="drawerTab" bg-color="surface" color="primary" grow height="50" class="border-bottom flex-shrink-0">
        ...
        <div class="flex-grow-1 overflow-y-auto pa-6 bg-slate-50">
          <v-window v-model="drawerTab" class="fill-height">
            <!-- PESTAÑA 1: ESTRUCTURA Y RECURSOS -->
            <v-window-item value="general" class="pa-1">
              <!-- Información Técnica -->
              <v-card class="rounded-xl border pa-4 mb-4 bg-white" elevation="2">
                <h3 class="text-subtitle-2 font-weight-black text-primary mb-3 uppercase">Detalles Técnicos</h3>
                <v-row dense>
                  <v-col cols="12" md="6" class="mb-2">
                    <span class="text-caption font-weight-bold text-slate-400 d-block">PERIODICIDAD:</span>
                    <span class="text-body-2 font-weight-medium text-slate-800">{{ selectedProcedure.periodicidad || 'No definida' }}</span>
                  </v-col>
                  <v-col cols="12" md="6" class="mb-2">
                    <span class="text-caption font-weight-bold text-slate-400 d-block">PROCESO PADRE:</span>
                    <span class="text-body-2 font-weight-medium text-slate-800">{{ selectedProcedure.proceso?.nombre || 'Proceso relacionado' }}</span>
                  </v-col>
                  <v-col cols="12" class="mb-3">
                    <span class="text-caption font-weight-bold text-slate-400 d-block">OBJETIVOS:</span>
                    <span class="text-body-2 text-slate-700 bg-slate-50 pa-2 rounded d-block border border-dashed">{{ selectedProcedure.objetivos || 'No definido' }}</span>
                  </v-col>
                  <v-col cols="12" class="mb-2">
                    <span class="text-caption font-weight-bold text-slate-400 d-block">ALCANCE:</span>
                    <span class="text-body-2 text-slate-700 bg-slate-50 pa-2 rounded d-block border border-dashed">{{ selectedProcedure.alcance || 'No definido' }}</span>
                  </v-col>
                </v-row>
              </v-card>
              ...
              <v-card class="rounded-xl border pa-4 mb-4 bg-white" elevation="2">
                <h3 class="text-subtitle-2 font-weight-black text-primary mb-2 uppercase">Instalaciones y Dependencias físicas</h3>
                <p class="text-caption text-grey-darken-1 mb-3">Oficinas y dependencias donde se ejecuta o supervisa el procedimiento.</p>
                <div class="d-flex flex-wrap ga-2">
                  <v-chip
                  ...
                  <span v-if="!selectedProcedure.instalaciones || selectedProcedure.instalaciones.length === 0" class="text-caption text-grey italic">No hay dependencias físicas asociadas a este procedimiento.</span>
                </div>
              </v-card>

              <!-- Herramientas Digitales y Materiales -->
              <v-card class="rounded-xl border pa-4 bg-white" elevation="2">
                <h3 class="text-subtitle-2 font-weight-black text-primary mb-2 uppercase">Sistemas y Equipamiento</h3>
                <p class="text-caption text-grey-darken-1 mb-3">Recursos tecnológicos y materiales obligatorios para realizar las tareas.</p>
                
                <div class="text-caption font-weight-bold text-slate-500 mb-2">SOFTWARE / SISTEMAS:</div>
                <div class="d-flex flex-wrap ga-2 mb-4">
                  ...
                  <span v-if="activeSistemas.length === 0" class="text-caption text-grey italic">Ningún sistema de información asociado.</span>
                </div>

                <div class="text-caption font-weight-bold text-slate-500 mb-2">EQUIPAMIENTO / MATERIALES:</div>
                <div class="d-flex flex-wrap ga-2">
                  <v-chip
                  ...
                  <span v-if="activeEquipos.length === 0" class="text-caption text-grey italic">Ningún equipamiento o material físico asociado.</span>
                </div>
              </v-card>
            </v-window-item>

            <!-- PESTAÑA 2: CALIDAD Y NORMATIVA -->
            <v-window-item value="calidad" class="pa-1">
              <!-- Leyes y Normas -->
              <v-card class="rounded-xl border pa-4 mb-4 bg-white" elevation="2">
                <h3 class="text-subtitle-2 font-weight-black text-indigo-darken-4 mb-2 uppercase">Leyes, Reglamentos y Normas</h3>
                <p class="text-caption text-grey-darken-1 mb-3">Marco legal e institucional que ampara y regula la ejecución del trámite.</p>
                
                <v-list density="compact" class="bg-transparent pa-0">
                  <v-list-item v-for="norma in activeNormativas" :key="norma.id_normativa" class="border rounded-lg bg-slate-50 pa-3 mb-2">
                    <template v-slot:title>
                      <span class="text-subtitle-2 font-weight-bold text-slate-800">{{ norma.nombre }}</span>
                    </template>
                    <template v-slot:subtitle>
                      <div class="text-caption text-slate-500 mt-1">Código: <span class="font-weight-bold">{{ norma.codigo || 'S/C' }}</span></div>
                      <div v-if="norma.descripcion" class="text-caption text-slate-600 mt-1 italic">{{ norma.descripcion }}</div>
                    </template>
                    <template v-slot:append>
                      <v-btn v-if="norma.url" :href="norma.url" target="_blank" icon="mdi-open-in-new" variant="tonal" size="small" color="primary"></v-btn>
                    </template>
                  </v-list-item>
                </v-list>
                <div v-if="activeNormativas.length === 0" class="text-caption text-grey italic py-4 text-center">No hay normas legales vinculadas a este procedimiento.</div>
              </v-card>

              <!-- Indicadores de Calidad -->
              <v-card class="rounded-xl border pa-4 bg-white" elevation="2">
                <h3 class="text-subtitle-2 font-weight-black text-green-darken-4 mb-2 uppercase">Indicadores de Rendimiento (KPIs)</h3>
                <p class="text-caption text-grey-darken-1 mb-3">Métricas diseñadas para evaluar la eficiencia, tiempos de atención y calidad.</p>
                
                <v-row dense>
                  <v-col cols="12" v-for="ind in activeIndicadores" :key="ind.id_indicador" class="mb-3">
                    <v-card flat class="border rounded-lg pa-3 bg-slate-50">
                      <div class="d-flex justify-space-between align-center mb-1">
                        <span class="text-subtitle-2 font-weight-black text-slate-800">{{ ind.denominacion }}</span>
                        <v-chip size="x-small" color="green-darken-2" variant="flat" class="font-weight-bold">
                          Meta: {{ ind.meta || 'S/M' }}
                        </v-chip>
                      </div>
                      <div class="text-caption text-slate-600 mb-2">{{ ind.descripcion || 'Sin descripción' }}</div>
                      <v-row no-gutters class="text-caption bg-white border pa-2 rounded">
                        <v-col cols="6" class="border-right">
                          <span class="font-weight-bold text-slate-400 uppercase d-block text-xxs">Fórmula:</span>
                          <span class="font-mono text-grey-darken-3">{{ ind.formula || 'No definida' }}</span>
                        </v-col>
                        <v-col cols="3" class="border-right pl-2">
                          <span class="font-weight-bold text-slate-400 uppercase d-block text-xxs">Frecuencia:</span>
                          <span>{{ ind.frecuencia || 'Mensual' }}</span>
                        </v-col>
                        <v-col cols="3" class="pl-2">
                          <span class="font-weight-bold text-slate-400 uppercase d-block text-xxs">Unidad:</span>
                ...
                <div v-if="activeIndicadores.length === 0" class="text-caption text-grey italic py-4 text-center">No hay indicadores de rendimiento asociados.</div>
              </v-card>
            </v-window-item>

            <!-- PESTAÑA 3: MATRIZ DE FLUJO -->
            <v-window-item value="flujo" class="pa-1 fill-height d-flex flex-column">
              <v-card class="rounded-xl border bg-white flex-grow-1 overflow-hidden d-flex flex-column" elevation="2">
                <div class="pa-4 border-bottom bg-white flex-shrink-0 d-flex align-center justify-space-between flex-wrap ga-2">
                  <div>
                    <h3 class="text-subtitle-2 font-weight-black text-slate-800 uppercase mb-1">Pasos y Diagrama de Flujo</h3>
                    <p class="text-caption text-grey-darken-1">Revise la secuencia de operaciones en formato tabla o diagrama de carriles (swimlanes).</p>
                  </div>
                  <!-- Conmutador de vista -->
                  <v-btn-toggle v-model="flowViewMode" mandatory color="primary" density="compact" class="rounded-lg">
                    <v-btn value="table" prepend-icon="mdi-table-large" class="text-caption font-weight-bold">Tabla</v-btn>
                    <v-btn value="diagram" prepend-icon="mdi-sitemap" class="text-caption font-weight-bold">Diagrama</v-btn>
                  </v-btn-toggle>
                </div>

                <div class="flex-grow-1 overflow-auto bg-slate-50 position-relative w-100">
                  <v-progress-linear
                  ...
                  <div v-else-if="activeFlow.length === 0" class="pa-8 text-center text-caption text-grey">
                    No se encontraron operaciones registradas para este flujo en el backend.
                  </div>

                  <!-- MODO TABLA -->
                  <v-table v-else-if="flowViewMode === 'table'" class="flow-matrix-table drawer-table">
                    <thead>
                      <tr class="bg-white border-bottom text-uppercase text-caption font-weight-bold">
                        <th width="50" class="text-center font-weight-bold">Paso</th>
                        <th width="150" class="font-weight-bold">Entradas / Requisitos</th>
                        <th width="170" class="font-weight-bold">Actividad (Verbo)</th>
                        <th width="200" class="font-weight-bold">Tarea / Descripción</th>
                    ...
                    <tbody>
                      <tr v-for="row in activeFlow" :key="row.nro">
                        <td class="text-center text-caption font-weight-black text-primary">{{ row.nro }}</td>
                        <td class="text-caption text-slate-600">{{ row.requisitos }}</td>
                        <td class="text-caption font-weight-bold text-slate-800">{{ row.actividad }}</td>
                        <td class="text-caption text-slate-700">{{ row.tarea }}</td>
                        <td class="text-caption text-slate-700">
                          <v-chip size="x-small" color="primary" variant="tonal" class="font-weight-bold">
                            {{ row.responsableCargo }}
                          </v-chip>
                        </td>
                        <td class="text-caption text-red-darken-3">{{ row.riesgo }}</td>
                        <td class="text-caption text-green-darken-3">{{ row.control }}</td>
                      </tr>
                    </tbody>
                  </v-table>
                  ...
                  >
                    ...
                    <div v-if="allDisplayCargos.length === 0" class="text-center pa-10 text-grey-darken-1">
                      ...
                      <p class="font-weight-bold">No hay cargos de responsabilidad asignados en este procedimiento.</p>
                      <p class="text-caption">El diagrama requiere al menos una operación con un responsable para poder estructurar los carriles.</p>
                    </div>
                    ...
                    <div v-else ref="scrollWrapper" class="diagram-scroll-wrapper" style="position: relative; display: inline-block; min-width: 100%;">
                        ...
                        <tbody>
                          <tr v-for="row in activeFlow" :key="row.nro">
                            ...
                            >
                              ...
                              <div 
                                v-if="Number(row.responsableCargoId) === Number(cargo.id_cargo)"
                      ...
                      >
                        ...
                        <path 
                          ...
                          stroke-width="2"
                          fill="none"
                          :marker-end="path.isReturn ? 'url(#arrow-hist-return)' : 'url(#arrow-hist)'"
                        />
                      </svg>
                    </div>
                  </div>
...
const fetchAllProcedimientos = async () => {
  try {
    const response = await axios.get("http://localhost:3000/procesos/procedimientos");
...
const fetchAllCargoProcesos = async () => {
  try {
    const response = await axios.get("http://localhost:3000/procesos/cargo-procesos");
...
const loadProcedureFlow = async (procId) => {
  ...
  try {
    const [opRes, actRes, tarRes, cargoRes, riesgosRes, controlesRes, reqRes] = await Promise.all([
      axios.get("http://localhost:3000/flujo/operaciones").catch(() => ({ data: [] })),
      axios.get("http://localhost:3000/flujo/actividades").catch(() => ({ data: [] })),
      axios.get("http://localhost:3000/flujo/tareas").catch(() => ({ data: [] })),
      axios.get("http://localhost:3000/flujo/operacion-cargos").catch(() => ({ data: [] })),
      axios.get("http://localhost:3000/recursos/riesgos").catch(() => ({ data: [] })),
      axios.get("http://localhost:3000/recursos/controles").catch(() => ({ data: [] })),
      axios.get("http://localhost:3000/recursos/requisitos").catch(() => ({ data: [] }))
    ]);
    ...
    const mappedRows = matrixOps.map(op => {
      ...
      if (responsable.cargo) {
        respCargoName = responsable.cargo.denominacion || responsable.cargo.nombre;
        respCargoId = responsable.cargo.id_cargo;
      } else if (responsable.id_cargo) {
        respCargoId = responsable.id_cargo;
        const matchingCargo = mppStore.cargos.find(c => Number(c.id_cargo) === Number(responsable.id_cargo));
        respCargoName = matchingCargo ? matchingCargo.denominacion || matchingCargo.nombre : `Cargo #${responsable.id_cargo}`;
      }
      ...
      return {
        ...
        accionId: actividad.id_accion || (actividad.accion?.id_accion || op.id_accion),
  ...
  nextTick(() => {
    setTimeout(() => {
      ...
      nodes.forEach((node, index) => {
        ...
        const row = activeFlow.value.find(r => Number(r.nro) === rowNro) || activeFlow.value[index];
        pts.push({
          cx,
          cy,
          w: rect.width,
...
const getActionVisuals = (accionId) => {
  ...
  if (!accion || !accion.figura) return { icon: "mdi-circle", color: "primary", colorHex: "#6366f1", codigoFigura: "rectangulo" };
  ...
  if (nombreAccion.includes("inicio") || nombreAccion.includes("empezar") || nombreAccion.includes("comenzar") || nombreAccion.includes("start")) { 
    color = "success"; 
    colorHex = "#10b981"; 
  } else if (nombreAccion.includes("fin") || textMatchAny(nombreAccion, ["terminar", "concluir", "archivar", "end"])) { 
    color = "error"; 
    colorHex = "#ef4444"; 
  } else if (textMatchAny(nombreAccion, ["decisión", "validar", "aprob", "revisar", "control", "analiz", "decid"])) { 
...
const selectedProcedure = computed(() => {
  ...
  return allProcedimientos.value.find(p => p.id_procedimiento === selectedProcedureId.value);
});
...
const activeNormativas = computed(() => {
  ...
  return (mppStore.normativas || []).filter(n => {
    if (!n) return false;
    const hasInArray = Array.isArray(n.id_procedimientos) && n.id_procedimientos.includes(id);
    const hasInRelations = Array.isArray(n.procedimientos) && n.procedimientos.some(p => p && Number(p.id_procedimiento) === id);
...
const activeIndicadores = computed(() => {
  ...
  return list.filter(k => {
    if (!k) return false;
    const hasInArray = Array.isArray(k.id_procedimientos) && k.id_procedimientos.includes(id);
    const hasInRelations = Array.isArray(k.procedimientos) && k.procedimientos.some(p => p && Number(p.id_procedimiento) === id);
    return hasInArray || hasInRelations;
  });
});
...
const activeSistemas = computed(() => {
  ...
  return (mppStore.sistemasInformacion || []).filter(s => {
    if (!s) return false;
    const hasInArray = Array.isArray(s.id_procedimientos) && s.id_procedimientos.includes(id);
    const hasInRelations = Array.isArray(s.procedimientos) && s.procedimientos.some(p => p && Number(p.id_procedimiento) === id);
    return hasInArray || hasInRelations;
  });
});
...
const activeEquipos = computed(() => {
  ...
  return (mppStore.equipos || []).filter(e => {
    if (!e) return false;
    const hasInArray = Array.isArray(e.id_procedimientos) && e.id_procedimientos.includes(id);
    const hasInRelations = Array.isArray(e.procedimientos) && e.procedimientos.some(p => p && Number(p.id_procedimiento) === id);
    return hasInArray || hasInRelations;
  });
});
...
const allDisplayCargos = computed(() => {
  ...
  const cargoIds = Array.from(new Set(rows.map(r => Number(r.responsableCargoId)).filter(id => !isNaN(id) && id > 0)));
  
  const list = [];
  cargoIds.forEach(id => {
    const cargo = (mppStore.cargos || []).find(c => Number(c.id_cargo) === id);
    if (cargo) {
      let unitId = null;
...
const getProcedimientosGrouped = (procesoId) => {
  ...
  if (statusFilter.value !== "Todos") {
    filtered = filtered.filter(p => (p.estado || "Activo") === statusFilter.value);
  }
  ...
  filtered.forEach(p => {
    ...
    nombreBase = nombreBase.replace(/[\s\-_(]*v(er)?(sión)?\.?\s*\d+(\.\d+)*\)?$/gi, "").trim();
...
const getProcessResponsible = (procesoId) => {
  ...
  if (responsible.cargo) return responsible.cargo.denominacion || responsible.cargo.nombre;
  const cargoObj = (mppStore.cargos || []).find(c => Number(c.id_cargo) === Number(responsible.id_cargo));
  return cargoObj ? cargoObj.denominacion || cargoObj.nombre : `Cargo #${responsible.id_cargo}`;
};
...
watch([drawerOpen, drawerTab, selectedProcedureId], () => {
  if (drawerOpen.value && drawerTab.value === "flujo" && selectedProcedureId.value) {
...
watch([drawerOpen, drawerTab, flowViewMode, () => activeFlow.value, scrollWrapper], () => {
  if (drawerOpen.value && drawerTab.value === "flujo" && flowViewMode.value === "diagram" && activeFlow.value.length > 0 && scrollWrapper.value) {
