<template>
  <div class="fill-height overflow-y-auto pa-6 d-flex justify-center align-center w-100 bg-slate-50">
    <v-card
      class="rounded-xl border-top-primary overflow-hidden elevation-12 w-100 my-4"
      max-width="1100"
    >
    <!-- Encabezado unificado y serio como la cabecera -->
    <v-toolbar
      color="surface"
      flat
      class="px-6 py-4 border-bottom d-flex align-center"
      height="auto"
    >
      <div>
        ...
        <div class="text-caption text-slate-500 mt-1 d-flex flex-column ga-1">
          <div>
            Proceso activo:
            <v-chip size="x-small" color="primary" variant="tonal" class="font-weight-bold text-uppercase ml-1">
              {{ procesoNombre }}
            </v-chip>
          </div>
          <div>
      ...
      <v-window v-model="activeTab">
        ...
        <v-window-item value="instalaciones">
          <div class="mb-4">
            <h3 class="text-h6 font-weight-bold text-grey-darken-3">
              Unidades / Instalaciones Participantes
            </h3>
            <p class="text-caption text-grey-darken-1">
              Seleccione de la lista de dependencias físicas u oficinas donde se realiza o supervisa este procedimiento. Los cambios se guardan automáticamente.
            </p>
          </div>

          <div class="mt-4">
            <v-autocomplete
              v-model="selectedInstallationIds"
              :items="mppStore.unidades"
              item-title="nombre"
              item-value="id_unidad"
              label="Buscar y asociar dependencias físicas..."
              ...
              no-data-text="No se encontraron dependencias"
            >
        ...
        <v-window-item value="indicadores">
          <div class="d-flex align-center justify-space-between mb-4">
            <div>
              ...
              <p class="text-caption text-grey-darken-1">
                Métricas y fórmulas para medir la eficiencia y calidad del
                trámite.
              </p>
            </div>
            <v-btn
          ...
          <v-table class="rounded-xl border bg-surface overflow-hidden">
            ...
            <tbody>
              <tr v-if="indicadoresFiltrados.length === 0">
                <td colspan="5" class="text-center py-8 text-grey">
                  No hay indicadores registrados para este procedimiento.
                  ¡Agrega uno!
                </td>
              </tr>
              <tr v-for="item in indicadoresFiltrados" :key="item.id_indicador">
                ...
                <td class="text-caption font-mono text-grey-darken-1">
                  {{ item.formula || "No definida" }}
                </td>
                <td
                  class="text-body-2 text-center font-weight-bold text-green-darken-2"
                >
                  {{ item.meta || "-" }}
                </td>
                ...
                <td class="text-center">
                  <v-btn
                    ...
                    color="primary"
                    class="mr-1"
                    @click="openModal('indicador', item)"
                  ...
                  <v-btn
                    ...
                    variant="tonal"
                    color="error"
                    @click="deleteResource('indicador', item.id_indicador)"
                    ><v-icon size="16">mdi-trash-can-outline</v-icon></v-btn
                  >
                </td>
              </tr>
            </tbody>
        ...
        <v-window-item value="normativas">
          <div class="d-flex align-center justify-space-between mb-4">
            <div>
              <h3 class="text-h6 font-weight-bold text-grey-darken-3">
                Marco Legal y Directivas
              </h3>
              <p class="text-caption text-grey-darken-1">
                Leyes, reglamentos y normativas de calidad que amparan el
                procedimiento.
              </p>
            </div>
            <v-btn
            ...
            <tbody>
              <tr v-if="normativasFiltradas.length === 0">
                <td colspan="4" class="text-center py-8 text-grey">
                  No hay normativas registradas para este procedimiento. ¡Agrega
                  una!
                </td>
              </tr>
              <tr v-for="item in normativasFiltradas" :key="item.id_normativa">
                ...
                <td class="text-center">
                  <v-btn
                    ...
                    color="primary"
                    class="mr-1"
                    @click="openModal('normativa', item)"
                    ><v-icon size="16">mdi-pencil</v-icon></v-btn
                  >
                  <v-btn
                    icon
                    ...
                    variant="tonal"
                    color="error"
                    @click="deleteResource('normativa', item.id_normativa)"
                    ><v-icon size="16">mdi-trash-can-outline</v-icon></v-btn
                  >
                </td>
              </tr>
        ...
        <v-window-item value="sistemas">
          <div class="d-flex align-center justify-space-between mb-4">
            <div>
              ...
              <p class="text-caption text-grey-darken-1">
                Herramientas digitales e informáticas requeridas en los pasos
                del flujo.
              </p>
            </div>
            <v-btn
            ...
            <tbody>
              <tr v-if="sistemasFiltrados.length === 0">
                <td colspan="3" class="text-center py-8 text-grey">
                  No hay sistemas registrados para este procedimiento. ¡Agrega
                  uno!
                </td>
              </tr>
              <tr
                v-for="item in sistemasFiltrados"
                :key="item.id_sistema_informacion"
              >
                <td class="text-body-2 font-weight-bold text-grey-darken-3">
                  {{ item.nombre }}
                </td>
                ...
                <td class="text-center">
                  ...
                  <v-btn
                    ...
                    color="error"
                    @click="
                      deleteResource('sistema', item.id_sistema_informacion)
                    "
                    ><v-icon size="16">mdi-trash-can-outline</v-icon></v-btn
                  >
                </td>
        ...
        <v-window-item value="equipos">
          <div class="d-flex align-center justify-space-between mb-4">
            <div>
              <h3 class="text-h6 font-weight-bold text-grey-darken-3">
                Equipos, Hardware y Herramientas
              </h3>
              <p class="text-caption text-grey-darken-1">
                Maquinaria, hardware u otros recursos materiales asignados.
              </p>
            </div>
            <v-btn
              color="primary"
          ...
          <v-table class="rounded-xl border bg-surface overflow-hidden">
            ...
            <tbody>
              <tr v-if="equiposFiltrados.length === 0">
                <td colspan="3" class="text-center py-8 text-grey">
                  No hay equipos registrados para este procedimiento. ¡Agrega
                  uno!
                </td>
              </tr>
              <tr v-for="item in equiposFiltrados" :key="item.id_equipos">
                ...
                <td class="text-center">
                  ...
                  <v-btn
                    ...
                    variant="tonal"
                    color="error"
                    @click="deleteResource('equipo', item.id_equipos)"
                    ><v-icon size="16">mdi-trash-can-outline</v-icon></v-btn
                  >
                </td>
              </tr>
          ...
          <v-form ref="formRef" class="mt-2">
            ...
            <v-row dense v-if="modalType === 'indicador'">
              <v-col cols="12">
                <v-text-field
                  ...
                  label="Denominación del Indicador"
                  variant="outlined"
                  density="compact"
                  required
                ></v-text-field>
              ...
              <v-col cols="12">
                <v-text-field
                  v-model="formData.metodo_verificacion"
            ...
            <v-row dense v-if="modalType === 'normativa'">
              ...
              <v-col cols="12">
                <v-text-field
                  ...
                  label="URL / Enlace del Documento"
                  variant="outlined"
                  density="compact"
                ></v-text-field>
              </v-col>
            ...
            <v-row dense v-if="modalType === 'equipo'">
              <v-col cols="12">
                <v-text-field
                  v-model="formData.nombre"
                  label="Nombre del Equipo / Recurso"
                  variant="outlined"
                  density="compact"
                  required
                ></v-text-field>
...
onMounted(async () => {
  try {
    ...
    if (!mppStore.sistemasInformacion.length)
    ...
    console.error(
      "Error cargando dependencias en formulario complementario:",
      err,
    );
...
const selectedInstallationIds = computed({
  get() {
    if (!currentProcedimiento.value || !currentProcedimiento.value.instalaciones) return [];
    return currentProcedimiento.value.instalaciones.map((u) => Number(u.id_unidad || u.id));
  },
  async set(newIds) {
    const pId = Number(props.procedimientoId);
    try {
      console.log(`📡 [Form-Comp] Sincronizando instalaciones para procedimiento ${pId}:`, newIds);
      await mppStore.updateProcedimiento(pId, { id_instalaciones: newIds.map(Number) });
      await loadProcedimientoDetails();
...
const indicadoresFiltrados = computed(() => {
  const pId = Number(props.procedimientoId);
  return mppStore.indicadores.filter(
    (item) =>
...
const normativasFiltradas = computed(() => {
  const pId = Number(props.procedimientoId);
  return mppStore.normativas.filter(
    (item) =>
      Array.isArray(item.procedimientos) &&
      item.procedimientos.some((p) => Number(p.id_procedimiento) === pId),
  );
...
const sistemasFiltrados = computed(() => {
  const pId = Number(props.procedimientoId);
  return mppStore.sistemasInformacion.filter(
    (item) =>
      Array.isArray(item.procedimientos) &&
      item.procedimientos.some((p) => Number(p.id_procedimiento) === pId),
  );
...
const equiposFiltrados = computed(() => {
  const pId = Number(props.procedimientoId);
  return mppStore.equipos.filter(
    (item) =>
      Array.isArray(item.procedimientos) &&
      item.procedimientos.some((p) => Number(p.id_procedimiento) === pId),
  );
...
const modalTitle = computed(() => {
  switch (modalType.value) {
    case "indicador":
      return "Indicador (KPI)";
    case "normativa":
      return "Marco Normativo";
    case "sistema":
      return "Sistema de Información";
    case "equipo":
      return "Equipo";
    default:
      return "Recurso";
...
const saveModal = async () => {
  ...
  try {
    ...
    if (mode.value === "create") {
      ...
      showNotification(
        `${modalTitle.value} actualizado exitosamente`,
        "success",
      );
    }
